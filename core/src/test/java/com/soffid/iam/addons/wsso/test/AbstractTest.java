...
public abstract class AbstractTest extends AbstractHibernateTest
{
	...
	{
		System.setProperty("soffid.ui.maxrows", "999999"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (cfg == null)
		{
			...
			OsType os = xarxaSvc.findOSTypeByName("ALT");
			if (os == null)
			{
				os = new OsType();
				os.setDescription("Others");
				os.setName("ALT");
				xarxaSvc.create(os);
			}
	...
	protected void setUp() throws Exception
	{
		...
		try {
			ServiceLocator.instance().init ("testBeanRefFactory.xml");
